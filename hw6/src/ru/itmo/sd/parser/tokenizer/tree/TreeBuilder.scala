package ru.itmo.sd.parser.tokenizer.tree

import ru.itmo.sd.parser.tokenizer.{BraceToken, OpToken, Token}

import java.util
import ru.itmo.sd.parser.tokenizer.OpToken._
...
class TreeBuilder(protected val tokens: util.List[Token]) {
  ...
  private def buildByPriority(priority: Int): Token = {
    ...
    var left = if (isMax) buildBrace() else buildByPriority(priority + 1)
      ...
      tokens get (index + 1) match {
        case op: OpToken if getPriorityLevel(op) == priority =>
          ...
          val right = buildByPriority(priority + 1) match {
            case t: Token => t
            case _ =>
              println("Warning: right part of expression nof found for " + op.getOperation)
              tokens.get(index)
          }
